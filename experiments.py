...
if __name__ == "__main__":
    ...
    result = subprocess.run(
        f"python main.py --model image_space --img test_images/* --add_bounding_boxes --num_clusters={num_clusters_image_space} --output_dir "
        f"test_results/results_image_space_kmeans_gt --clustering=kmeans --gamma={gamma_image_space} --gt_num_clusters "
        f"--preds_filename=test_results/results_image_space_kmeans_gt/image_space_kmeans_gt.json",
        shell = True, stdout = subprocess.PIPE)
    output = result.stdout.decode('utf-8')
    f = open(f"test_results/results_image_space_kmeans_gt/image_space_kmeans_gt.txt", "w+")
    f.write(output)
    f.close()
    ...
    result = subprocess.run(
        f"python main.py --model trained --img test_images/* --add_bounding_boxes --num_clusters={num_clusters_trained} --output_dir "
        f"./test_results/results_trained_kmeans_gt --clustering=kmeans --gamma=0 --gt_num_clusters "
        f"--preds_filename=test_results/results_trained_kmeans_gt/trained_kmeans.json",
        shell = True, stdout = subprocess.PIPE)
    output = result.stdout.decode('utf-8')
    f = open(f"./test_results/results_trained_kmeans_gt/trained_kmeans.txt", "w+")
    f.write(output)
    f.close()
    ...
    result = subprocess.run(
        f"python main.py --model random --img test_images/* --add_bounding_boxes --num_clusters={num_clusters_random} --output_dir "
        f"test_results/results_random_kmeans_gt --clustering=kmeans --gamma={gamma_random} --gt_num_clusters "
        f"--preds_filename=test_results/results_random_kmeans_gt/random_kmeans.json",
        shell = True, stdout = subprocess.PIPE)
    output = result.stdout.decode('utf-8')
    f = open(f"test_results/results_random_kmeans_gt/random_kmeans.txt", "w+")
    f.write(output)
    f.close()
